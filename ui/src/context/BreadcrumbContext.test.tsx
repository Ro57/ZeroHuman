...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Zerohuman", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Zerohuman",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Zerohuman");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Zerohuman");
    expect(buildDocumentTitle([], null)).toBe("Zerohuman");
  });
});
